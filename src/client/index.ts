import type {ModalPopup, CoubsType, CoubsJson} from './types/index';
...
export class DownloadCoubClient {
	...
	async getJSON(url: string): Promise<CoubsJson> {
		return await $.get(url) as CoubsJson;
	}
...
/* istanbul ignore next */
if (window.location.host === 'coub.com') new DownloadCoubClient().execute();
